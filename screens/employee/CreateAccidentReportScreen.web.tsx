...
// Add TypeScript type for styles
type StylesType = {
  container: ViewStyle;
...
const CreateAccidentReportScreen = () => {
  ...
  const getFileIcon = (filename: string | null) => {
    if (!filename)
      return <IconButton icon="file" size={24} iconColor="#F44336" />;

    const extension = filename.split(".").pop()?.toLowerCase();

    switch (extension) {
      case "pdf":
        return <IconButton icon="file-pdf-box" size={24} iconColor="#F44336" />;
      case "doc":
      case "docx":
        return (
          <IconButton icon="file-word-box" size={24} iconColor="#2196F3" />
        );
      case "jpg":
      case "jpeg":
      case "png":
        return <IconButton icon="file-image" size={24} iconColor="#4CAF50" />;
      default:
        return <IconButton icon="file" size={24} iconColor="#F44336" />;
    }
  };
